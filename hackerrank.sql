...
--6. The names of all the Japanese cities in the CITY table. The COUNTRYCODE for Japan is JPN.
SELECT NAME
FROM CITY
WHERE COUNTRYCODE='JPN';
...
SELECT DISTINCT CITY
FROM STATION 
WHERE LOWER(left(city,1)) IN ('a','o','e','i','u') AND LOWER(right(city,1)) IN ('a','o','e','i','u');
...
OR ( lower(city) NOT LIKE 'a%' and lower(city) NOT LIKE 'e%' and lower(city) NOT LIKE 'i%' and lower(city) NOT LIKE 'o%' and lower(city) NOT LIKE 'u%' ) ;
